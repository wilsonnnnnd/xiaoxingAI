...
export const register = (email: string, password: string, display_name?: string, ui_lang?: 'en' | 'zh', notify_lang?: 'en' | 'zh', invite_code?: string) =>
  api.post<{ access_token: string; token_type: string }>('/auth/register', { email, password, display_name, ui_lang, notify_lang, invite_code }).then(r => r.data)

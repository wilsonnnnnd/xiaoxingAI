    ...
    <div className="flex flex-col h-full p-5 gap-6 min-w-0 max-w-6xl mx-auto w-full">
          ...
          {ASSIGN_KEYS.map(k => (
            <div key={k.id} className="grid grid-cols-1 sm:grid-cols-[200px_1fr] items-center gap-3">
              ...
              <Select
                ...
                options={files.map(f => ({ label: `${FILE_LABELS[f] ?? f} (${f})`, value: f }))}
              />
            </div>
          ))}
      ...
      <div className="flex flex-col gap-0">
        ...
        <div className="bg-[#1e2330] border border-[#2d3748] rounded-b-xl rounded-tr-xl p-5 flex flex-col gap-4 shadow-xl">
          <div className="flex items-center justify-between text-[10px] text-[#475569] uppercase tracking-widest font-bold">
            <span>{current ?? '—'}</span>
            <span>{t('prompts.editor_hint')}</span>
          </div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <Button onClick={handleSave} loading={saving} disabled={loading || !isDirty} className="px-8">
              {t('prompts.btn.save')}
            </Button>
            <Button
              variant="primary"

...
Separately, the system can use an optional **Router LLM** to decide which tools to call during Telegram chat.
...
| `OPENAI_API_KEY` | Required when `LLM_BACKEND=openai` | `sk-...` |
...
The Router LLM is a smaller model used for **tool intent detection** in Telegram chat. If the router is unavailable, the system falls back to keyword matching.
...
- Main chat prompt: `app/prompts/chat.txt`
...
- **401 / unauthorized**: check `OPENAI_API_KEY` when using `LLM_BACKEND=openai`.

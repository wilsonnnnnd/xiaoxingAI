...
每个注册用户独立运行一个 Gmail 轮询 Worker。新邮件到达后经过 2 阶段 AI 流水线处理，并通过确定性模板渲染推送到 Telegram（不会为 Telegram 文案额外调用 LLM）。
...
| 步骤 | Prompt / 代码 | 输出 |
|------|--------------|------|
| 1. 分类 | `app/prompts/gmail/email_analysis.txt` | category + priority + 英文一句话 summary + action_needed |
| 2. 摘要 | `app/prompts/gmail/email_summary.txt` | 结构化英文 summary + key_points |
| 3. Telegram 通知 | `app/skills/gmail/telegram_format.py` | HTML 格式通知文本（确定性渲染，tokens=0） |
...
去重检查使用批量查询（每轮一次性查询已处理的 email_id 集合），以降低数据库与线程池压力。
...
| `GMAIL_WORKER_IO_CONCURRENCY` | `8` | Worker IO 并发上限 |
| `GMAIL_WORKER_IO_MAX_WORKERS` | `12` | Worker 专用线程池大小 |
| `GMAIL_WORKER_START_JITTER_MAX` | `15` | 首轮错峰最大秒数 |
| `GMAIL_WORKER_START_BUCKETS` | `12` | 首轮错峰分桶数量 |

设置页也提供每用户独立的配置覆盖。

另见：

- [Worker 运行逻辑 →](../../doc/worker-runtime.md)

...
每个注册用户独立运行一个 Gmail 轮询 Worker。新邮件到达后经过 4 阶段 AI 流水线处理，并推送到 Telegram。
...
- 只获取符合搜索语法的邮件（默认：`is:unread in:inbox`）
...
| `GMAIL_POLL_QUERY` | `is:unread in:inbox` | Gmail 搜索语法 |
| `GMAIL_POLL_MAX` | `20` | 每次最多处理邮件数 |

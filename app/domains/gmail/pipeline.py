...
from .telegram_format import render_telegram_message
...
def analyze_email(
    ...
    attachment_line = (
        f"\n\nAttachments: {int(attachment_count)} file(s) attached"
...
def summarize_email(
    ...
    parsed = extract_json_with_repair(
        ...
        user_id=user_id,
        purpose="email_summary_json_repair",
    )
    ...
    if _cjk_count(s + " " + kp_text) >= 3:
        try:
            raw2, tokens2 = call_llm(
                prompt,
                max_tokens=256,
                use_cache=False,
                user_id=user_id,
                purpose="email_summary_retry",
                source="gmail",
            )
...
def process_email(
    ...
    try:
        ...
        raise RuntimeError(f"[Step 1 - Analysis] {str(e)}")
    ...
    try:
        ...
        raise RuntimeError(f"[Step 2 - Summarization] {str(e)}")
    ...
    try:
        ...
        raise RuntimeError(f"[Step 3 - Telegram Message Generation] {str(e)}")

    total_tokens = analysis.get("tokens", 0) + summary.get("tokens", 0) + tg_tokens
    return {
        "subject": subject,

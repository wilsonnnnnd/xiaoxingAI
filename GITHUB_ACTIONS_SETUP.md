...
GitHub Actions 运行在 GitHub 的云服务器上，它用 SSH 私钥登录你的云服务器，执行一段 shell 脚本完成部署。需要提前做四件事：

1. **服务器上**：为 GitHub Actions 单独生成一对 SSH 密钥
2. **服务器上**：把 FastAPI 后端交给 systemd 管理（不再手动跑 uvicorn）
3. **服务器上**：允许 `ubuntu` 用户无密码 `sudo systemctl restart`（CI 无法输入密码）
4. **GitHub 上**：在仓库 Secrets 里存入连接信息和私钥

---

## 第一步：在服务器上生成专用 SSH 密钥

> **为什么要单独生成一对，而不用已有的密钥？**  
> GitHub Secrets 里存的是私钥，单独生成一对可以随时撤销，不影响你平时登录服务器的密钥安全。

在服务器上执行：

```bash
# 生成 ed25519 密钥，注释写 github-actions 方便识别，不设密码（-N ""）
ssh-keygen -t ed25519 -C "github-actions" -f ~/.ssh/id_ed25519 -N ""

# 把公钥加入授权列表，允许持有私钥的人登录
...
最后一行会看到类似：
```
ssh-ed25519 AAAA...kKnh github-actions
```

### 查看私钥内容（稍后要复制到 GitHub）

```bash
cat ~/.ssh/id_ed25519
```

输出的全部内容（从 `-----BEGIN OPENSSH PRIVATE KEY-----` 到 `-----END OPENSSH PRIVATE KEY-----`）需要完整保存，下一步用。

---

## 第二步：创建 systemd 服务（服务器上操作）
...
# 验证语法正确（输出 parsed OK 即成功）
sudo visudo -c
```

> **安全说明：** 只授权了 `restart xiaoxingai` 和 `reload nginx` 这两个具体命令，而不是 `ALL`，遵循最小权限原则。

---

## 第四步：在 GitHub 仓库添加 Secrets
...
| `PORT` | `22` | SSH 端口 |
| `SSH_PRIVATE_KEY` | 第一步生成的私钥全文 | 见下方注意事项 |

### SSH_PRIVATE_KEY 填写注意事项

在服务器上执行 `cat ~/.ssh/id_ed25519`，把输出**完整复制**粘贴到 Secret 里：

```
-----BEGIN OPENSSH PRIVATE KEY-----
b3BlbnNzaC1rZXktdjEAAAA...（中间若干行）
-----END OPENSSH PRIVATE KEY-----
```

⚠️ 常见填错导致 `ssh: no key found` 的情况：
- 首行或末行被截断
- 粘贴时多了空行或空格
- 只复制了中间部分，没有 `BEGIN` / `END` 行
- 在值的前后加了引号
